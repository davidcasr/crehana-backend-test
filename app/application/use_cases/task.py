...
from ...domain.exceptions import (
    TaskListNotFoundException, TaskNotFoundException, TaskTitleAlreadyExistsException,
...
class TaskUseCases:
    ...
    def create_task(self, task_list_id: int, title: str, description: str = None, 
        ...
        if not task_list:
            raise TaskListNotFoundException(f"Task list with ID {task_list_id} not found")
        ...
        if self.task_repository.exists_by_title_in_list(title, task_list_id):
            raise TaskTitleAlreadyExistsException(f"Task with title '{title}' already exists in this list")
    ...
    def get_filtered_tasks(self, task_list_id: int, status: Optional[TaskStatus] = None, 
        ...
        if status or priority:
            tasks = self.task_repository.get_filtered_tasks(task_list_id, status=status, priority=priority)
        else:
    ...
    def update_task(self, task_id: int, title: str = None, description: str = None,
        ...
        if title and title != task.title:
            if self.task_repository.exists_by_title_in_list(title, task.task_list_id, exclude_id=task_id):
                raise TaskTitleAlreadyExistsException(f"Task with title '{title}' already exists in this list")
    ...
    def get_tasks_with_stats(self, task_list_id: int, status: Optional[TaskStatus] = None, 
                            priority: Optional[TaskPriority] = None) -> dict:
        ...
        # Get filtered tasks
        if status or priority:
            filtered_tasks = self.task_repository.get_filtered_tasks(task_list_id, status=status, priority=priority)
        else:
        ...
        completed_tasks = len([task for task in all_tasks if task.status == StatusEnum.COMPLETED])
