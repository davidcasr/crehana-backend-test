...
from ...domain.exceptions import (
    TaskListNotFoundException, TaskListNameAlreadyExistsException
)
...
class TaskListUseCases:
    ...
    def create_task_list(self, name: str, description: str = None) -> TaskList:
        ...
        if self.task_list_repository.exists_by_name(name):
            raise TaskListNameAlreadyExistsException(f"Task list with name '{name}' already exists")
    ...
    def update_task_list(self, task_list_id: int, name: str = None, description: str = None) -> TaskList:
        """Update a task list."""
        task_list = self.task_list_repository.get_by_id(task_list_id)
        if not task_list:
            raise TaskListNotFoundException(f"Task list with ID {task_list_id} not found")
