...
class SQLTaskListRepository(TaskListRepository):
    ...
    def get_by_id(self, task_list_id: int) -> Optional[TaskList]:
        ...
        db_task_list = self.db.query(TaskListModel).filter(
            TaskListModel.id == task_list_id
        ).first()
    ...
    def _to_entity(self, db_task_list: TaskListModel) -> TaskList:
        ...
        # Get task count
        task_count = self.db.query(TaskModel).filter(
            TaskModel.task_list_id == db_task_list.id
        ).count()
